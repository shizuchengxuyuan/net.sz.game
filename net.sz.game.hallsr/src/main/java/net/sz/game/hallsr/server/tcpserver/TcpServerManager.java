package net.sz.game.hallsr.server.tcpserver;
...
public class TcpServerManager {
    ...
    public INettyHandler getiNettyHandler() {
        return iNettyHandler;
    }

    public void setiNettyHandler(INettyHandler iNettyHandler) {
        this.iNettyHandler = iNettyHandler;
        if (addBindTcpServer != null) {
            addBindTcpServer.setNettyHandler(iNettyHandler);
        }
    }

    public void startServer(int port) {
        addBindTcpServer = NettyPool.getInstance().addBindTcpServer("0.0.0.0", port, NettyCoder.getDefaultCoder(), iNettyHandler);
